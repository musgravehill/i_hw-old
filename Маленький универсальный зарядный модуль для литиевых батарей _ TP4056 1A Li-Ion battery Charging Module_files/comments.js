var lsCmtTreeClass = new Class({
	...
	responseNewComment: function(idTarget,typeTarget,objImg,selfIdComment,bNotFlushNew) {
		...
		var idCommentLast=this.idCommentLast;
		 ...
		 if (objImg)
			 objImg.setStyle('background-image','url('+DIR_STATIC_SKIN+'/images/toolbar-btn-update-loading.gif)');	
		...
		JsHttpRequest.query(        	
        	...
        	function(result, errors) {        		
        		...
        		if (objImg)
        			objImg.setStyle('background-image','url('+DIR_STATIC_SKIN+'/images/toolbar-btn-update.png)'); 
