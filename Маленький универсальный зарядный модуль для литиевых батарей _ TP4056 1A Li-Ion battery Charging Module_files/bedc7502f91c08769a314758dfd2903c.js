...
var lsVoteClass = new Class({
        ...
        onVote: function(result, errors, thisObj) {            
        	...
        	if (result.bStateError) {
                ...
                var divCount=divVoting.getSiblings('.'+thisObj.options.classes_element.count);
...
var lsFavouriteClass = new Class({
        ...
        onToggle: function(result, errors, thisObj) {            
        	...
        	if (result.bStateError) {
                ...
                thisObj.objFavourite.removeClass(thisObj.options.classes_action.active);
               
                /* У нас есть возможность сменить надпись на элементе при добавлении товара в избранное */
                if (thisObj.objFavourite.get('data-toggle-0') && thisObj.objFavourite.get('data-toggle-1') )
                {
                	if (result.bState)
                		thisObj.objFavourite.set('text',thisObj.objFavourite.get('data-toggle-1'));
                	else
                 		thisObj.objFavourite.set('text',thisObj.objFavourite.get('data-toggle-0'));
                         		
                }	
                
                if (result.bState) {
                	thisObj.objFavourite.addClass(thisObj.options.classes_action.active);
                }
        	}      
        }
...
function closeButtonMenu(menuId)
{
	$('button_menu_'+menuId).hide();
}


function showButtonMenu(menuId,element)
{
	$$('.button_menu').hide();

	var pos = $(element).getPosition();
	var parent_dim = $(element).getParent().getParent().getSize();
	var my_dim = $(element).getParent().getSize();
	pos.y += my_dim.y+1;

	$('button_menu_'+menuId).setPosition(pos);
	//$('button_menu_'+menuId).setStyle('width',-5+parent_dim.x+'px');
	$('button_menu_'+menuId).show();
	
}

function toggleButtonMenu(menuId,element)
{
    $$('.button_menu').hide();
...
function ajaxBlogInfo(idBlog,idSku) 
{ 
	if (idSku)
	{	
		ImageUrlChange('/uploads/images/custom/ajax-loader.gif');
	}
    else
	{
	    idSku=null;
	...
	JsHttpRequest.query(
    	'POST '+DIR_WEB_ROOT+'/include/ajax/blogSkuInfo.php',                       
        { idBlog: idBlog, idSku: idSku, security_ls_key: LIVESTREET_SECURITY_KEY  },
        function(result, errors) 
        {  
        	var get_image = false;
        	...
        	if (result.bShowError)
        	{
        		msgErrorBox.alert('Error',result.sShowError);
        		
        		/* Убираем загрузчик, только если мы его до этого поставили */
        		if (idSku)
        			ImageUrlChange();
            ...
            */
        	...
        	if (! result.bSkuStateError) 
            {        
            	/*
           	  	if (result.sTitle && $('sku_title')) 
           		{
           			$('sku_title').set('value',result.sTitle);            		
           		}
            	*/
        		
            	if (result.sPrice && $('sku_price')) 
            	{
            		$('sku_price').set('value',result.sPrice);            		
            	}

            	if (result.sImageUrl && $('sku_image')) 
            	{
            		$('sku_image').set('value',result.sImageUrl);
            		ImageUrlChange(result.sImageUrl);
            		get_image = true;	
            	}

            	if (result.sTags && $('sku_tags')) 
            	{
            		$('sku_tags').set('value',result.sTags);            		
            	}

            	if (result.sSkuUrl && $('sku_url')) 
            	{
            	   $('sku_url').set('value',result.sSkuUrl);            
            	}              	
            	
                if (result.bHasSimilar)
                {
                    noticeText = '<div class="notifi"> <!-- Стили для уведомлений при добавлении товара -->';
                    noticeText += '    <div class="notifi-title">Внимание!</div>';
                    noticeText += '    <div class="notifi-content">';
                    noticeText += '    На нашем сайте уже есть <a href="/blog/'+result.aSimilarTopics.id+'.html">обзор данного товара:</a> '+result.aSimilarTopics.title+'. Вы можете оставить отзыв к <a href="/blog/'+result.aSimilarTopics.id+'.html">существующему обзору</a>, а не создавать новый обзор.';
                    noticeText += '    </div>';
                    noticeText += '</div> <!-- /.notifi -->';
			
					if ($('similar_sku'))
					{			
					    $('similar_sku').set('html',noticeText); 
		                	    $('similar_sku').show();	
					}
                }	
                else if ($('similar_sku'))
...
/* Функции для открытия / закрытия описаний категорий */ 

function open_category_description_block()
{
    var     container = $('category-description-container'),
            wrapper = $('category-description-wrapper'),
            sidebar = $('category-description-sidebar');

    var is_processed = $('category-description-container').get('data-processed');

    if (is_processed === null)
    { 
        /* This is first time run of this function,so set mark of run */
        $('category-description-container').set('data-processed',1);
        
        /* Define common variables */
        var descriptionHeight = 0,
            topicsHeight = 0,
            averageTopicHeight = 550,
            topic = null,
            items = 1;

            
        /* Make DOM manupulation */    
        sidebar.adopt($$('.sort-link'));
        sidebar.adopt(container.getNext('.topic'));
     
        descriptionHeight = wrapper.getSize().y;
        topicsHeight = sidebar.getSize().y;                    
        
        while ( descriptionHeight - averageTopicHeight > topicsHeight )  
        {
            items++;
            topic = container.getNext('.topic');
            if (topic == null)
            {
               break;
            }
            sidebar.adopt(topic);
            topicsHeight = sidebar.getSize().y;
            averageTopicHeight = topicsHeight / items;
        } 
                         
    } // end of first run


    wrapper.adopt($('category-description'));
    container.removeClass('state-closed'); 
    container.addClass('state-opened');                                        

}

function close_category_description_block()
{
    var     container = $('category-description-container');

    $('category-description-first-id').adopt($('category-description'));
    container.removeClass('state-opened'); 
    container.addClass('state-closed');                
}
...
function ajaxSkuPreview(skuUrl, skuTitle, skuImageUrl, skuPrice, reviewTitle, divPreview) {

    var shopName = $('blog_id').getSelected()[0].get('html');
    var skuUrl = $(skuUrl).value;
    var skuTitle =  $(skuTitle).value;
    var skuImageUrl = $(skuImageUrl).value;
    var skuPrice = $(skuPrice).value;
    var skuCategory = $('sku_category').getSelected()[0].get('html');
    var reviewTitle = $(reviewTitle).value;
    var topicText = $('sku_text').value;

    JsHttpRequest.query(
        'POST '+DIR_WEB_ROOT+'/include/ajax/skuPreview.php',
        { shopName: shopName, skuUrl: skuUrl, skuTitle: skuTitle, skuImageUrl: skuImageUrl, skuPrice: skuPrice, skuCategory: skuCategory, reviewTitle: reviewTitle, topicText: topicText, security_ls_key: LIVESTREET_SECURITY_KEY, save: true },
        function(result, errors) {
            if (!result) {
                msgErrorBox.alert('Error','Please try again later');
            }
                        ...
                        '<img class="product_image" src=' + result.sSkuImageUrl + '>' +
                            '<div class="price_go2_same">' +
                                ...
                                '<ul class="go2_same">' +
                                    ...
                                    '<li><a href="#">Найти похожие товары на search<em>sku</em>.ru</a></li>' +
                                '</ul>' +
...
{
	oMultiUpload =  new FancyUpload2($('form_multi_load_img'), $('upload-status-box'), { // options object
			...
			path: '/engine/lib/external/FancyUpload/Swiff.Uploader.swf',
			...
			target: 'progress-browse-button',
			
			// onLoad is only called if all went well with Flash
			onLoad: function() {
			...
			onFileSuccess: function(file, response) 
			{
				...
				if (json.js.bStateError)
				{
					msgErrorBox.alert(json.js.sMsgTitle,json.js.sMsg);
				}	
				else
				{
					...
					var img = new Element('img', {'width': '100', 'height' : '60', 'src' : json.js.sFilePath}).inject($('multi-upload-images-container'), 'bottom');
					img.addEvent('click', function(event){
						lsPanel.putText('sku_text',json.js.sText);
					});
			...
			onFail: function(error) {
				switch (error) {
					case 'hidden': // works after enabling the movie and clicking refresh
						msgErrorBox.alert('Error','To enable the embedded uploader, unblock it in your browser and refresh (see Adblock).');
						break;
					case 'blocked': // This no *full* fail, it works after the user clicks the button
						msgErrorBox.alert('Error','To enable the embedded uploader, enable the blocked Flash movie (see Flashblock).');
						break;
					case 'empty': // Oh oh, wrong path
						msgErrorBox.alert('Error','A required file was not found, please be patient and we fix this.');
						break;
					case 'flash': // no flash 9+ :(
						msgErrorBox.alert('Error','To enable the embedded uploader, install the latest Adobe Flash plugin.');
				}
			}
...
function showMultiImgUploadForm() {	
	if (Browser.Engine.trident) {
		//return true;
	}	
...
function checkSkuCategory(source)
{
	var sku_category = $('sku_category').getSelected();
	var sku_sub_category = $('sku_sub_category').getSelected();

	/* открывает или закрывает субкатегорию в зависимости от значения основной */
	if (source == 'main')
	{
		if (sku_category && (sku_category.get('value') >0))
			$('sku_sub_category_holder').show();
		else
			$('sku_sub_category_holder').hide();
	}	
	
	/* Проверяет, чтобы не была выбрана основная и суб категории одновременно */
	if (sku_category && sku_sub_category)
	{
		var sku_category_id = parseInt(sku_category.get('value'));
		var sku_sub_category_id = parseInt(sku_sub_category.get('value'));
		
		var sku_category_pid = parseInt(sku_category.get('data-pid'));
		var sku_sub_category_pid = parseInt(sku_sub_category.get('data-pid'));
		
	
		/* Выбрана как основная, так и дополнительная категория */
		if ( (sku_category_id > 0) && (sku_sub_category_id > 0) )
		{
			var show_error = false;
			
			/* Одна из категорий имеет родителем другую  */
			if (!show_error && (sku_sub_category_pid >0) && (sku_sub_category_pid == sku_category_id))
				show_error = true;			

			if (!show_error && (sku_category_pid >0) && (sku_category_pid == sku_sub_category_id))
				show_error = true;	
...
Slimbox.scanPage = function() {
        ...
        $$("div.topic img")
        	.each(function(el)
        	{
        		var imagesUrl = 'http://images.mysku.ru/uploads/images/';
        		...
        		if ( el.get('rel') == 'lbox')
            	{
        			...
        			if (src.test(regExp))
        			{
    		        	...
    		        	alink = new Element('a', {href:src.replace(regExp,imagesUrl+'original/')});
    		        	alink.wraps(el);
...
/* Переключение версий */
function change_display_mode(theme)
{
    /* Устанавливаем куку */
...
/* Обработка событий для Google Analytics */
function ga_event_click(event)
{
	var event_category = this.get("data-event-category");
	var event_action   = this.get("data-event-action");
	var event_label    = this.get("data-event-label");
	
	if (event_category && event_action)
	{
		var link_href      = this.get("href");
		var link_target    = this.get("target");
			link_target = (link_target && !link_target.match(/^_(self|parent|top)$/i)) ? link_target : false;
		
	    var hitCallBack = function(link, target){
	    
	        target ? window.open(link, target) : window.location.href = link;
	    };

		if (event_label)
			ga("send","event",event_category,event_action,event_label,{"hitCallback": hitCallBack(link_href, link_target)});
		else
			ga("send","event",event_category,event_action,{"hitCallback": hitCallBack(link_href, link_target)});
...
window.addEvent("domready",function()
{
	$$(".ga-event").each(
		function(element)
		{
			var event_type     = element.get("data-event-type");
			var event_category = element.get("data-event-category");
			var event_action   = element.get("data-event-action");
			var event_label    = element.get("data-event-label");
		
			/* Все обязательные данные на месте */
			if (event_type && event_category && event_action)
			{
				switch(event_type)
				{
					case "show":
					...
					case "click":	
						element.addEvent('click',ga_event_click);
...
function showLoginForm() {	
	...
	/* Do not show form for mobile devices */
...
window.addEvent('domready', function() 
{
	var spoilerFxSliders = new Array();
	
	var allSpoilers = $$('div.spoiler div');
    var id = 0;
    
    allSpoilers.each(function(el) 
    {
        if (el['className']=='spoiler-title') 
        {
            id = $random(10, 99)+''+$random(10, 99)+''+$random(10, 99)+''+$random(10, 99);
            el.setProperty('id', 'st'+id); /* Spoiler Title */
            el.setProperty('title', 'Удерживайте клавишу Alt при клике, чтобы открыть все спойлеры.');
            el.set('html','<span>'+el.innerHTML+'</span>');
        }
        
        if (el['className']=='spoiler-body') 
        {
            el.setProperty('id', 'sb'+id); /* Spoiler Body */
        }
    });

    var allSpoilersTitles = $$('body div.spoiler-title');
    
    allSpoilersTitles.each(function(el) 
    {
        var id_ = el['id'];
        var id = id_.slice(2, 10);
    	
        var spoilerVerticalSlide = new Fx.Slide('sb'+id, {
            onComplete: function(spoilerVerticalSlide){
                $(spoilerVerticalSlide).getParent().getParent().toggleClass('active');
            }
        });

        spoilerFxSliders.push(spoilerVerticalSlide);

        if ($('sb'+id).getStyle('display') == 'none') 
        {
        	spoilerVerticalSlide.toggle();
            (function(){
                $('sb'+id).setStyle('display', 'block');
            }).delay(100);
        }
    	...
    	el.addEvent('click', function(event)
    	{
    		...
    		{
    			// Open all spoilers
    			for (var i = 0; i < spoilerFxSliders.length; i++) 
    			{
    				spoilerFxSliders[i].show();	
    			}
    		}	
    		else
    		{	
...
;
function OpenAddComplainForm(sUserName,iTopicId,sTopicName)
{
	if(iTopicId > 0)
	{
		$('complain_topic_id').setProperty('value',iTopicId);
		$('complain_topic_name').set('html',sTopicName);
		$('complain_author_name').set('value',sUserName);
		$('complainWindow').show();
...
function AddComplain()
{
	var jsonRequest = new Request.JSON(
			{url: 'http://'+window.location.host+'/complain/add/', 
			 onSuccess: function(data)
			 	{
					...
					else
					{
						...
						{
							var n=data.errmsg.length;
							for(var i=0;i<n;i++)
							{
								errmsg+=data.errmsg[i];
							}
						}
						else
						{
						...
						msgErrorBox.alert('Ошибка',errmsg);
					}
			 	}
			}).post({'complain_topic_id': $('complain_topic_id').getProperty('value'),
					 'complain_text' : $('complain_text').value,
					 'complain_author_name': $('complain_author_name').getProperty('value')}
					);
...
function CloseComplainForm(sFormId)
{
	...
	;
function OpenAddLinksForm(iTopicId,sTopicName)
	{
		if(iTopicId > 0)
		{
			$('rl_src_topic_id').setProperty('value',iTopicId);
			$('rl_src_topic_name').set('html',sTopicName);
			$('rl_dst_topic_url').set('value','');
			$('cerabox').show();
	...
	{
		var jsonRequest = new Request.JSON(
			{url: 'http://'+window.location.host+'/reviewlinks/add/', 
			 onSuccess: function(data)
			 	{
					if(data.status=='ok')
					{
			...
			}).post({'src_topic_id': $('rl_src_topic_id').getProperty('value'),
					 'dst_topic_url': $('rl_dst_topic_url').getProperty('value')}
					);
	}
	
	function CloseForm(sFormId)
	{
		$(sFormId).hide();
	}
	
	function DeleteReviewLink(sLinkId,iContainerId)
	{
		if(confirm('Удалить ссылку?'))
		{
			var jsonRequest = new Request.JSON(
				{url: 'http://'+window.location.host+'/reviewlinks/delete/'+sLinkId, 
				 onSuccess: function(data)
				 	{
					 	if(data.status=='ok')
						{
							msgNoticeBox.alert('Удалено','Ссылка успешно удалена.');			
							$('rl_link_id_'+iContainerId).hide();
						}
					 	else
						{
							msgErrorBox.alert('Ошибка','Произошла ошибка при удалении ссылки.');
						}
	...
	function ApproveReviewLink(sLinkId,iContainerId)
	{
		var jsonRequest = new Request.JSON(
				{url: 'http://'+window.location.host+'/reviewlinks/approve/'+sLinkId, 
				onSuccess: function(data)
					{
						if(data.status=='ok')
						{
							msgNoticeBox.alert('Сохранено','Ссылка одобрена.');		
							$('rl_link_id_'+iContainerId).hide();
						}
						else
						{
							msgErrorBox.alert('Ошибка','Произошла ошибка.');
						}
